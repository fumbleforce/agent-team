...
| Runs an organisation autonomously | Autonomy levels for the PM (`observe`, `suggest`, `act`), daily spend caps, auto-merge with deterministic gates, quarantine on uncertain execution; when quarantine holds a project the coordinator's own watchdog comments once on the owner inbox issue with the held job ids, their recorded reason and the project's dashboard address, and once more when a job completes again, so a stall reaches the owner without any model session | Multi-team orchestration (one blueprint per department, one channel per organisation), budgets and policies as data, and an explicit owner-set autonomy ceiling per project; this stays gated on pilots, isolation and per-worker credentials |
