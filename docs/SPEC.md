...
- Memory lifecycle: filed by an agent, confirmed by the PM or a human, then promoted to a page, marked stale (no hits in 60 days or contradicted) or retired. Hits increment when a memory is injected or returned by search.
- `assemble(scopes, roles, cap)` in `packages/coordinator/src/knowledge/assemble.ts`: role read-first pages, then confirmed memories ranked by scope match, hits and recency, cut at the token cap, with the injected ids recorded on the turn.
- Search is `SearchPort` over pages, memories, messages and issues, filtered by the caller's access. Semantic search is optional through `VectorPort`, with embeddings from whichever provider is configured; its absence degrades to lexical search.
...
- Trackers are the board of record. `tasks` mirror tracker issues by polling (private deployments cannot receive webhooks); `external_refs` holds the identifier, URL and remote version; on conflict the remote wins for state, title and labels, and local-only fields (assignee agent, links, thread) are never overwritten.
