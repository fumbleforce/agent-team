...
import { claudeAgents, claudeAuth, claudeEnvironment, claudeInvocation, claudeStopReason, claudeSystemPrompt, validateEngine } from './engines.mjs';
...
const shared = { instructions: [], agent: {
  ...
  'team-dev': { mode: 'subagent', description: 'Dev', prompt: 'Build', permission: { task: 'deny', 'linear_*': 'deny' } },
...
test('engine names are validated', () => {
  ...
  assert.throws(() => validateEngine('codex'), /Unknown engine/);
});
...
test('shared subagent permissions map to Claude tool restrictions', () => {
  ...
  assert.deepEqual(agents['team-dev'].disallowedTools, ['Agent', 'mcp__linear']);
...
test('coordinator and ideation invocations differ in tools, MCP exposure and agents', () => {
  const coordinator = claudeInvocation({ ideate: false, prompt: 'cycle', model: 'sonnet', systemPromptFile: '/run/system-prompt.md', shared });
  ...
  assert.deepEqual(JSON.parse(coordinator.args[coordinator.args.indexOf('--mcp-config') + 1]).mcpServers.linear.url, 'https://mcp.linear.app/mcp');
  ...
  assert.ok(coordinator.args.includes('Bash(gh pr merge:*)') && coordinator.args.includes('mcp__linear'));
