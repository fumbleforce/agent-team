...
test('a raised issue leads somewhere: the PM is told how to make it a task and who there is, the page says who is on it, and a person can make the call themselves', async () => {
  ...
  try {
    ...
    const listed = async () => ((await call('/api/projects/shop/issues', { cookie })).json.issues as { number: number; task: { id: string; key: string; state: string; assigneeAgentId: string | null }; pending: { agentId: string; running: boolean } | null }[]).find(issue => issue.number === raised.number)!;

    // What is raised is a task from the start: it waits in the board's inbox, and the page can say the PM is up next.
    assert.deepEqual([(await listed()).pending, (await listed()).task.state, raised.taskId], [{ agentId: pm.id, running: false }, 'inbox', (await listed()).task.id]);
