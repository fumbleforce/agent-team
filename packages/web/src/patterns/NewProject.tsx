...
export function NewProject({ projects }: { projects: ProjectNode[] }) {
  ...
  async function create(form: FormData) {
    ...
    try {
      ...
      window.location.assign(`/p/${created.slug}/integrations`);
    } catch (failure) { setError(failure instanceof ApiError ? failure.message : 'The project could not be created'); }
    finally { setBusy(false); }
  }

  return (
    <Dialog open={open} onOpenChange={setOpen} title="New project" description="It starts with the default team of four. Next you connect its code, task board and chat, step by step." trigger={<Button variant="dashed" block>+ New project</Button>}>
