...
interface Item { kind: 'decision' | 'quarantine' | 'delivery' | 'proposal' | 'blocked'; id: string; projectId: string; title: string; detail: string; since: number; taskKey: string | null; href: string | null; canDecide: boolean; about?: { taskHref: string | null; taskTitle: string | null; who: string | null; doing: string | null; lastReport: string | null; branch: string | null; changeUrl: string | null } }
...
function NeedCard({ item, project, onDone }: { item: Item; project: string; onDone(): void }) {
  ...
  return (
    <Card tone={item.kind === 'blocked' ? 'raised' : 'decision'} className="flex flex-col gap-2.5">
      ...
      {item.about?.lastReport && <Text size="small" tone="muted">Last report: {item.about.lastReport}</Text>}
