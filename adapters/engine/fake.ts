...
export const fake: EngineAdapter = {
  ...
  prepare: (spec, _turnDir, env) => ({ bin: process.execPath, args: [SCRIPT, spec.kind], input: spec.prompt, env, files: [] }),
  parse(line, state) {
    let event: { type?: string; kind?: string; title?: string; detail?: string; session?: string; tokensIn?: number; tokensOut?: number; costUsd?: number; summary?: string };
    ...
    return [{ seq: state.nextSeq++, kind, title: event.title, ...(event.detail ? { detail: event.detail } : {}), status: 'ok' }];
  },
  classifyExit: (exit, state) => (state.limited ? 'rate-limited' : exit.code === 0 ? 'completed' : 'crashed'),
};
