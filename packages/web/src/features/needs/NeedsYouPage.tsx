...
interface Item { kind: 'decision' | 'quarantine' | 'delivery' | 'proposal' | 'blocked' | 'system'; id: string; projectId: string; title: string; detail: string; since: number; taskKey: string | null; href: string | null; canDecide: boolean; raw?: string | null; mergeRefused?: boolean; about?: { taskHref: string | null; taskTitle: string | null; who: string | null; doing: string | null; lastReport: string | null; branch: string | null; changeUrl: string | null } }
const LABEL: Record<Item['kind'], { chip: string; tone: ChipTone }> = { decision: { chip: 'Your call', tone: 'review' }, quarantine: { chip: 'Cut off mid-work', tone: 'stop' }, delivery: { chip: 'Cut off mid-merge', tone: 'stop' }, proposal: { chip: 'Proposal', tone: 'review' }, blocked: { chip: 'Blocked', tone: 'attention' }, system: { chip: 'Holding the team up', tone: 'stop' } };
      ...
      {!item.canDecide ? <Text size="caption" tone="muted">Someone who may decide for this project has to settle this.</Text> : (
        <>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {item.href && <Link href={item.href}><Text size="small" tone="accent">{item.kind === 'proposal' ? 'Read and decide →' : item.kind === 'system' ? 'Put it right →' : 'Open the discussion →'}</Text></Link>}
          </div>
        </>
      )}
    </Card>
