...
test('valid delivery merges once pinned to head and verifies actual merge commit', async () => {
  ...
  assert.equal(result.checkEnforcement, 'github-required');
  assert.match(result.checkSource, /GitHub-required/);
...
test('default and explicit github-required enforcement never fall back on lookup failures', async () => {
