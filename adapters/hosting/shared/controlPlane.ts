...
export async function startControlPlane(options: { publicUrl: string; env?: NodeJS.ProcessEnv }) {
  ...
  const coordinator = await startCoordinator({ host: '0.0.0.0', port: Number(env.PORT ?? 4310), storage, machineToken, secureCookies: options.publicUrl.startsWith('https://') });
