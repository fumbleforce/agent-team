...
test('a message is posted, answered in character and posted back; failures propagate without retry', async t => {
  ...
  const tracker = fakeLinear(); const runs = [];
  ...
  const result = await answer({ tracker, manifest, role: 'team-pm', issue: 'FUM-10', message: 'Jeff, status?', cwd: dir, work: ['PM acceptance of FUM-16'], run, runDir: path.join(dir, 'chat') });
  assert.deepEqual(result, { reply: 'I am reviewing FUM-16, nothing blocks it.', ownerCommentId: 'c1', replyCommentId: 'c2' });
  assert.deepEqual(tracker.posted.map(p => p.body), ['Owner → Jeff (product manager): Jeff, status?', 'Jeff (product manager): I am reviewing FUM-16, nothing blocks it.']);
  ...
  await assert.rejects(answer({ tracker, manifest, role: 'team-dev', issue: 'FUM-10', message: 'hi', cwd: dir, run: async () => { throw new Error('Claude usage limit reached'); }, runDir: path.join(dir, 'chat') }), /usage limit/);
  assert.equal(tracker.posted.length, 3);
  await assert.rejects(answer({ tracker: fakeLinear({ failPost: true }), manifest, role: 'team-pm', issue: 'FUM-10', message: 'hi', cwd: dir, run: async () => assert.fail('no model without a posted message'), runDir: path.join(dir, 'chat') }), /Linear HTTP/);
  await assert.rejects(answer({ tracker, manifest, role: 'team-pm', issue: 'nope', message: 'hi', cwd: dir, run: async () => assert.fail('no model for an invalid issue'), runDir: dir }), /tracker issue/);
  await assert.rejects(answer({ tracker, manifest, role: 'nobody', issue: 'FUM-10', message: 'hi', cwd: dir, runDir: dir }), /Unknown team member/);
});
