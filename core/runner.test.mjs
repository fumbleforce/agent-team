...
function fixture(t, modes = ['ready']) {
  ...
  // The shared package mirrors the toolkit layout: core/ and adapters/ modules plus roles and prompts.
  for (const dir of ['core', 'adapters']) fs.cpSync(fileURLToPath(new URL(`../${dir}`, import.meta.url)), path.join(packageDir, dir), { recursive: true, filter: source => !source.endsWith('.test.mjs') });
  ...
  fs.writeFileSync(path.join(packageDir, 'roles.json'), JSON.stringify({ agent: {
...
test('CLI defaults, bounds, conflicts and strict report schema', () => {
  assert.deepEqual(parseArgs([]), { execute: false, cycles: 1, timeoutMinutes: 45, base: 'HEAD', publish: false, autoMerge: false, fetch: false });
...
test('engine and fetch options are validated', () => {
  assert.equal(parseArgs([]).engine, undefined, 'the manifest engine.default applies when no --engine is given');
  assert.equal(parseArgs(['--engine', 'claude']).engine, 'claude');
  assert.throws(() => parseArgs(['--engine', 'unknown-engine']), /Unknown engine/);
...
test('claude engine runs on the subscription login with shared roles, project instructions and Linear-only MCP', async t => {
  ...
  assert.deepEqual(Object.keys(JSON.parse(capture.args[capture.args.indexOf('--mcp-config') + 1]).mcpServers), ['tracker']);
...
test('claude usage limits block the run without retry; other engine failures fail', async t => {
  ...
  assert.match(journal.summary, /usage limit reached/); assert.match(journal.summary, /No retry or metered fallback/);
...
test('--project selects a target from any cwd and --issue pins selection and result', async t => {
  ...
  assert.match(prompt, /workspace ID matches before any writes/);
...
test('execute isolates dirty primary work, overlays only setup, retains evidence and worktree', async t => {
  ...
  assert.match(invocation.at(-1), /do NOT commit, push, create a pull request/);
  assert.match(invocation.at(-1), /NEVER automatically mark Done/);
  assert.match(invocation.at(-1), /at most TWO repair rounds/);
  assert.match(invocation.at(-1), /project project \(https:\/\/linear\.app\/test\/project\)/);
});
...
test('SIGINT and SIGTERM preserve interrupted journals and kill the group', async t => {
  for (const signal of ['SIGINT', 'SIGTERM']) {
    await t.test(signal, async t => {
      const f = fixture(t, ['hang']);
      const child = spawn(process.execPath, [path.join(f.packageDir, 'core', 'runner.mjs'), '--project', f.root, '--execute', '--cycles', '2'], { cwd: os.tmpdir(), env: f.env, stdio: 'ignore' });
