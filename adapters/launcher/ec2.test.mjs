...
import { create, userData } from './ec2.mjs';
import { createLauncher, LAUNCHER_KINDS } from './index.mjs';
...
test('user data writes the worker configuration, runs exactly one job and powers off', () => {
  ...
  assert.ok(script.includes(`node core/worker.mjs --config /etc/agent-team/worker.json --once --job ${job.id}`));
...
test('start tries spot then on-demand, tags the instance, and stop and status address it by id', async () => {
  ...
  const run = async args => {
    ...
    if (args[1] === 'run-instances') {
      const spec = JSON.parse(args[args.indexOf('--cli-input-json') + 1]);
      if (spec.InstanceMarketOptions?.MarketType === 'spot' && spotFails) throw new Error('InsufficientInstanceCapacity');
  ...
  const spec = JSON.parse(calls[1][calls[1].indexOf('--cli-input-json') + 1]);
  ...
  assert.ok(spec.TagSpecifications[0].Tags.some(tag => tag.Key === 'agent-team:job' && tag.Value === job.id));
  assert.ok(spec.TagSpecifications[0].Tags.some(tag => tag.Key === 'team' && tag.Value === 'devs'));
...
test('the index creates launchers by kind and rejects unknown kinds', async () => {
  assert.ok(LAUNCHER_KINDS.includes('local') && LAUNCHER_KINDS.includes('ec2'));
