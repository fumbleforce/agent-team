...
const SECRETS = ['AGENT_TEAM_TOKEN', 'AGENT_TEAM_DASHBOARD_PASSWORD', 'LINEAR_API_KEY', 'GITHUB_ISSUES_TOKEN', 'ANTHROPIC_API_KEY', 'GITLAB_TOKEN', 'GH_TOKEN', 'SLACK_BOT_TOKEN', 'SLACK_MCP_TOKEN', 'HUBSPOT_ACCESS_TOKEN', 'HUBSPOT_MCP_TOKEN', 'GOOGLE_DRIVE_MCP_TOKEN'];
...
// The intake serves every tracker whose credential is present; a project's manifest names its kind.
const trackers = trackersWithCredentials(process.env);
const intake = write('intake.json', { coordinatorUrl: 'http://127.0.0.1:4310', pollSeconds: Number(process.env.AGENT_TEAM_INTAKE_SECONDS ?? 60), trackers });
...
if (trackers.length) start('intake', 'core/intake.mjs', intake, 3000);
