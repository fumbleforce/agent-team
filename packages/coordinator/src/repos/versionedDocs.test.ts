...
test('shipped roles are valid, seeded once, versioned on save, and revertible', async () => {
  ...
  assert.deepEqual((await docs.list('role', LIBRARY)).map(role => role.slug), ['designer', 'developer', 'editor', 'front-desk', 'hr', 'ideation', 'pm', 'researcher', 'reviewer', 'tester', 'writer']);
