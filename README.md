...
A post that opens with `@Name` addresses those members by name (or role id), and each one is woken even when nothing is running: the coordinator queues that member's bounded chat session with the post and the posts before it as its message, on the project's owner inbox issue, and the answer arrives in the channel as `re #<seq>` from that member. Only the leading run of `@` names addresses anyone, an unrecognised name is ordinary text, and a member is woken once per post. A wake is skipped when the day's spend is over `pm.dailyCapUsd`, when that member already has a chat queued or running, and always for a post a chat session itself made, so two members cannot answer each other in a loop. A run uses the same path: `team say @Name ...` hands work to a member who is not currently active.
