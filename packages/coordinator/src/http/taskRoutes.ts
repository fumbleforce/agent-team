...
export function mountTaskRoutes(app: Hono<Env>, context: Context, turns: Turns) {
  ...
  app.get('/api/tasks/:id', async c => {
    ...
    const done = await db.selectFrom('turns').select(['id', 'agent_id', 'kind', 'state', 'summary', 'stop_reason', 'started_at', 'finished_at']).where('task_id', '=', task.id).orderBy('started_at', 'desc').limit(20).execute();
    ...
    const steps: Step[] = [
      ...
      // The one who holds the task now is the one picking it up: after a reassignment, the first worker's name must not stand here.
      { label: 'Picked up', who: working && !inbox ? `${name(working.agent_id) ?? '—'} · ${working.defer_reason ? 'held' : working.state === 'leased' ? 'working now' : 'up next'}` : firstWork ? name(firstWork.agent_id) ?? '—' : '—', agentId: working?.agent_id ?? firstWork?.agent_id ?? null, at: firstWork ? Number(firstWork.started_at) : null, state: at(Boolean(firstWork), !firstWork && Boolean(working) && !inbox) },
    ...
    return c.json({
      ...
      log: done.map(turn => ({ id: turn.id, agentId: turn.agent_id, kind: turn.kind, state: turn.state, summary: turn.summary, stopReason: turn.stop_reason, at: Number(turn.started_at) })),
