...
// Polls the tracker for owner-approved ideas and turns them into development jobs; refills the
// idea backlog with ideation jobs when the cooldown allows.
export async function pollProject({ manifest: raw, queue, tracker, now = Date.now }) {
  const full = raw.version === 2 && raw.scm ? raw : normalizeManifest(raw);
  const manifest = flatTracker(full);
  const client = tracker;
  const approvalStatus = client.approvalStatus ?? trackerAdapter(full.tracker.kind).approvalStatus;
  ...
  if (!overlapping) for (const issue of eligible) {
    ...
    await client.prepareApproved(manifest, issue.identifier);
...
export async function runIntake(config, { once = false, signal = new AbortController().signal,
  queue, tracker, clients = {}, now = Date.now, onError = () => console.error('Intake project poll failed; verify configuration, credentials, and service availability') } = {}) {
  ...
  // One client per tracker kind, created lazily from each manifest so mixed projects work.
  const clientFor = kind => { if (tracker) return tracker; clients[kind] ??= trackerClient(kind); return clients[kind]; };
  ...
  do {
    ...
    for (const [queueProjectId, read] of entries) {
      ...
      try {
        const manifest = normalizeManifest(read());
        if (manifest.queueProjectId !== queueProjectId) throw new Error('Intake project routing mismatch');
        await pollProject({ manifest, queue, tracker: clientFor(manifest.tracker.kind), now });
      } catch {
...
export async function main(args = process.argv.slice(2)) {
  ...
  if (!configPath) throw new Error('Usage: node core/intake.mjs --config intake.json [--once]');
  let config;
  try { config = JSON.parse(readFileSync(configPath, 'utf8')); } catch { throw new Error('Cannot read intake configuration'); }
  // Fail explicitly before starting if the configured trackers' keys are absent.
  const clients = {};
  for (const kind of config.trackers ?? [trackerAdapter().NAME]) clients[kind] = trackerClient(kind);
  const control = new AbortController(); const stop = () => control.abort();
  process.once('SIGINT', stop); process.once('SIGTERM', stop);
  try { await runIntake(config, { once, signal: control.signal, clients }); }
