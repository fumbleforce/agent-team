...
export function Field({ label, error, children }: { label: string; error?: string | undefined; children: ReactNode }) {
  return <label className="flex flex-col gap-1.5"><Text size="small" tone="muted">{label}</Text>{children}{error && <Text size="caption" tone="stop">{error}</Text>}</label>;
}
...
export function Textarea({ className, bare, ...rest }: TextareaHTMLAttributes<HTMLTextAreaElement> & { bare?: boolean }) {
