...
import { validateEngine, ENGINES, engineAdapter } from '../../engine/index.mjs';
...
// The dashboard config has no project registry; only conflicting settings are refused.
function flat(file, text, defaults) {
  let existing = {};
  if (text !== null) {
    try { existing = JSON.parse(text); } catch { throw new Error(`Invalid JSON config: ${file}`); }
    if (!object(existing)) throw new Error(`Invalid config: ${file}`);
  }
  for (const [name, value] of Object.entries(defaults)) {
    if (Object.hasOwn(existing, name) && !isDeepStrictEqual(existing[name], value)) throw new Error(`Conflicting ${name} in ${file}`);
  }
  return JSON.stringify({ ...existing, ...defaults }, null, 2) + '\n';
}
...
export function install({ project, key, repository, engine, install: write = false, home = homedir(), toolkit = toolkitRoot,
  ...
  resolveBinary = name => which(name), log = console.log } = {}) {
  ...
  // Every installed engine CLI joins the service PATH; the selected one must resolve.
  const binaries = Object.fromEntries(ENGINES.map(name => [name, resolveBinary(engineAdapter(name).BIN)]));
  for (const [name, binary] of Object.entries(binaries)) {
    if (binary === null && (engine ?? ENGINES[0]) === name) throw new Error(`Cannot resolve ${engineAdapter(name).BIN} on PATH`);
  ...
  for (const [role, defaults, registration] of [
    ...
    ['dashboard', { host: '127.0.0.1', port: 4311, coordinator: path.join(configDir, 'coordinator.json'), worker: path.join(configDir, 'worker.json') }]
  ]) {
    const file = path.join(configDir, `${role}.json`); const previous = readExisting(file);
    files.push({ file, previous, contents: role === 'dashboard' ? flat(file, previous, defaults) : config(file, previous, defaults, key, registration) });
  ...
  log('systemctl --user start agent-team-coordinator.service agent-team-worker.service agent-team-dashboard.service');
