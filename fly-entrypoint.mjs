...
// One container runs the control plane: coordinator, Linear intake (when a key is present) and the
// dashboard. Configuration is derived from environment/secrets; state lives on the mounted volume.
...
const env = { ...process.env, AGENT_TEAM_PUBLIC_BIND: '1', AGENT_TEAM_URL: 'http://127.0.0.1:4310' };
const children = [];
...
start('coordinator', 'queue.mjs', coordinator);
start('dashboard', 'dashboard.mjs', dashboard, 1500);
if (process.env.LINEAR_API_KEY) start('intake', 'linear-intake.mjs', intake, 3000);
