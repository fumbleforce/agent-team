...
Retain atomic claims, single writer per worktree, one delivery per project, lease validation on every worker and tool call, fail-stop workers and scoped quarantine after uncertain execution. Never auto-retry or auto-reassign uncertain work. A turn cut off by a restart of its own worker is not uncertain: that worker ends the turn's process itself when it comes back and still holds the worktree, so the work continues there after a pause; a delivery cut off mid-merge stays a person's call, because whether the merge went through cannot be known from the worker. Do not treat instructions or permission patterns as OS isolation. Publishing, merging and service activation require explicit authorization in the committed manifest.
