...
export const PROVIDERS: ProviderEntry[] = [
  {
    ...
    steps: ['On each worker machine install Claude Code: npm install -g @anthropic-ai/claude-code', 'Run "claude" there once, type /login and sign in with the account that holds the subscription.', RESTART],
  ...
  },
  {
    ...
    steps: ['On each worker machine install opencode: npm install -g opencode-ai', 'Create an API key at console.anthropic.com → API keys.', 'On the worker run "opencode auth login", choose Anthropic and paste the key. It is stored by opencode on that machine only.', 'Run "opencode models" to see the exact model names you may list below.', RESTART],
  ...
  },
  {
    ...
    steps: ['On each worker machine install opencode: npm install -g opencode-ai', 'In the worker\'s opencode settings file (~/.config/opencode/opencode.json) add the gateway as a custom provider with its address, as opencode.ai/docs/providers describes. Give it a short id such as "gateway".', 'Run "opencode auth login", choose Other, enter the same id and paste the gateway\'s key.', RESTART],
    fields: [
  ...
  },
  {
    ...
    steps: ['Install Ollama (ollama.com) where the models should run and pull a model, for example: ollama pull qwen2.5-coder', 'On each worker machine install opencode: npm install -g opencode-ai', 'In the worker\'s opencode settings file (~/.config/opencode/opencode.json) add Ollama as a provider with the id "ollama", as opencode.ai/docs/providers describes.', 'If Ollama runs on another machine, set OLLAMA_HOST on the worker to its address.', RESTART],
  ...
  },
  {
    ...
    steps: ['On each worker machine install Codex: npm install -g @openai/codex', 'Run "codex login" there and sign in with ChatGPT.', RESTART],
    fields: [models(['gpt-5-codex', 'gpt-5'], 'These are suggestions; use the names Codex shows under /model.'), CONCURRENCY, ...WINDOW],
  ...
  },
  {
    ...
    steps: ['On each worker machine install Codex: npm install -g @openai/codex', 'Create a key at platform.openai.com → API keys.', 'Run "codex login" on the worker and choose to sign in with an API key. Codex stores it on that machine only.', RESTART],
    fields: [models(['gpt-5-codex', 'gpt-5'], 'These are suggestions; use the names your key has access to.'), CONCURRENCY, ...WINDOW],
    credential: { label: 'OpenAI API key', runsOn: 'workers', login: 'codex login' },
  },
  {
    ...
    steps: ['On each worker machine install the Cursor command-line agent as cursor.com/cli describes.', 'Run "agent login" there and sign in.', RESTART],
    fields: [models(['auto'], '"auto" lets Cursor choose; "agent --help" on the worker lists other names.'), CONCURRENCY],
