...
| Goal | Today | Next |
| --- | --- | --- |
| Provisions itself, with permissions and a persistent controller | `agent-team init/deploy/status/destroy` builds the AWS control plane (EC2 or Fargate, EBS/EFS data, SSM secrets, scoped roles, permission boundary, dedicated network) and bakes worker images; `systemd` and `fly` hosting adapters cover a private host and Fly.io | A `hosting` adapter interface with the same `deploy/status/destroy` surface for Fly and a generic Docker host so the CLI picks the target from `--target`; per-integration IAM/OAuth scoping in the plan |
| Shared long-term memory and a channel between active agents | Git-backed project memory with search, assembly into prompts, proposals and PM curation; the team channel (this change) is read and posted to by every run, the PM and the owner through the coordinator | Cross-project memory scopes for organisation-wide facts; channel mentions that wake a specific member's chat job |
| Adapts to any set of personas and tasks | Team blueprints (this change): `roles.json`, `roster.json`, `agents/*.md` per blueprint, selected per control plane; `team.roles` selects subagents per project; `teams/research-desk` is a non-coding example | Blueprint selection per project rather than per control plane; a task-kind abstraction so a cycle need not be a Git worktree and a tracker issue |
...
| Any task, easy external integrations | Integrations (this change): Slack, Google Drive, HubSpot and any MCP server as manifest entries with per-role access, credentials as headers only, prompt notes and evidence; the tracker is already an adapter | More integration kinds (email, calendar, spreadsheets), OAuth device flows inside `agent-team init`, and a per-integration write log the owner can audit and revert |
...
- The cycle is still repository-shaped: one worktree, one tracker issue, one report file. A non-coding blueprint works inside a repository of documents; a task model without Git is the next step listed above.
