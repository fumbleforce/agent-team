...
export function createNeedsYou(context: Context, turns: Turns) {
  ...
  return {
    async list(): Promise<NeedsYouItem[]> {
      ...
      const tasks = new Map((await db.selectFrom('tasks').select(['id', 'key', 'title', 'project_id']).execute()).map(row => [row.id, row]));
      ...
      for (const row of await db.selectFrom('quarantines').selectAll().where('released_at', 'is', null).execute()) {
        ...
        items.push({ kind: 'quarantine', id: row.id, projectId, title: task ? `${task.key} stopped with an unknown outcome` : 'A worker’s copy of the repository is in an unknown state', detail: row.reason, since: Number(row.opened_at), taskKey: task?.key ?? null, href: null });
      }
      for (const row of await db.selectFrom('merge_queue').selectAll().where('state', '=', 'uncertain').execute()) {
        const task = tasks.get(row.task_id);
        items.push({ kind: 'delivery', id: row.id, projectId: row.project_id, title: `${task?.key ?? 'A task'}: the merge was cut off`, detail: row.reason ?? 'Check on the code host whether the change was merged, then say which it was. Merging for this project waits until you do.', since: Number(row.created_at), taskKey: task?.key ?? null, href: null });
      }
      ...
      for (const row of await db.selectFrom('tasks').select(['id', 'project_id', 'key', 'title', 'blocked_reason', 'updated_at']).where('state', '=', 'blocked').execute())
        items.push({ kind: 'blocked', id: row.id, projectId: row.project_id, title: `${row.key} is blocked`, detail: `${row.title}${row.blocked_reason ? ` — ${row.blocked_reason}` : ''}`, since: Number(row.updated_at), taskKey: row.key, href: null });
    ...
    async releaseQuarantine(userId: string, quarantineId: string, resolution: 'continue' | 'stop', note: string) {
      const released = await storage.transaction(async tx => {
        ...
        const published = await events.append(tx, [{ type: 'quarantine.released', category: 'audit', actorKind: 'user', userId, projectId: task?.project_id ?? turn?.project_id ?? null, taskId: task?.id ?? null, payload: { quarantineId, scope: row.scope, resolution, note } }]);
