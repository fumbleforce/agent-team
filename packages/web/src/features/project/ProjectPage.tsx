...
import { AppShell, BoardColumn, Composer, mentionOptions, Message, PageHeader, RailHeader, resolveAuthor, Sidebar, usePinnedTail } from '../../patterns';
...
function Discussion({ threadId, view, me }: { threadId: string; view: ProjectView; me: Me }) {
  ...
  const show = (next: 'discussion' | 'feed') => { if (next === 'discussion') tail.pin(); setShowing(next); try { localStorage.setItem('rail', next); } catch { /* a convenience only */ } };
  useEffect(tail.follow, [thread.data?.messages.length, showing]);
  ...
  return (
    <>
      ...
      <div ref={tail.container} onScroll={tail.onScroll} className="flex min-h-0 grow flex-col gap-3 overflow-y-auto px-4 py-3.5">
