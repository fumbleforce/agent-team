...
[SPEC.md](SPEC.md) is the design and [ROADMAP.md](ROADMAP.md) is what is built next and how it is measured. This page says what of it runs, how far each part has been proven, and what is still open. Statuses are about this repository's code: nothing is proven at scale until a pilot has run it on a real project.
...
| Continuity of a seat | Built and tested on a fake engine: a notebook per seat and a journal per task, both given to the turns they belong to on whatever worker; an owner whose turn ends with the task still in progress is given its next turn at once; three turns that edit nothing, or every twelfth turn on one task, bring the PM in; a turn that ran out of time continues once. No real engine has kept a notebook or a journal yet |
| Advice | Built and tested: `deliberation.propose` with `decides: "me"` asks up to two colleagues, and the owner of the work decides from their blocks in its next turn |
...
| Setting things up in the app | A project is created from the sidebar. Integrations are connected through a catalog of what the adapters support (`adapters/integration/catalog.ts`): each entry has its own steps, fields, credential check and, where the coordinator holds the credential, a live connection test. Keys and tokens are typed into the app (model provider keys, integration tokens the coordinator uses, the single sign-on client secret), sealed with AES-256-GCM under a key from `AGENT_TEAM_SECRET_KEY` or a private file next to the database, and take effect without a restart; a provider key travels to a worker only with a turn on that provider. This deviates from section 9.5 of the spec ("secrets stay on workers") at the owner's request. Code-host sign-ins for pushing still live on the workers. Models and IDs are picked from the product's own lists where it publishes one. No connection test has run against a real account |
| One object for what is raised and what is worked on | Per boards 05 and 18 of the canvas: a report (from a person, the product view, an agent) is a task from the start, in the board's Inbox column (state `inbox`), with the report's thread as the task's own. Its page is the same as any task's: a pipeline strip (raised, accepted, picked up, changed, review, done) with the step it is at lit, the report or brief, the triage or work log, and a rail with owner, reviewer, links and the thread. From the inbox a person accepts it to someone or to the backlog, merges it into another task, or declines it; the PM's triage does the same. The Issues tab is gone; old issue links lead to the task. The work log shows each turn's own summary; the canvas's structured change / also checked / verified cards need a structured report from agents, which is not built |
| Front desk | A seat that wears the `front-desk` role answers the owner first: what is written in the discussion and said to it directly goes to it as a reply turn with a briefing of what is going on (workers, who is on what, what waits and why, what needs the owner), and `desk.handover` passes work to the PM. It starts on the provider's quick middle model when a provider names one. The app's Ask panel takes speech and reads answers aloud with the browser's own speech features; the spec left voice out of v1. No real turn has run as the desk yet |
| Hosting: local, systemd, Fly, AWS | Local is used daily by the tests and `up`. The Fly and AWS entrypoints boot locally; neither has been deployed from this code. The AWS control plane is now handed its launcher, so queued work starts a worker there; that path is tested with fakes only. The AWS deploy plan and its CLI (`deploy aws`, plan by default, `--apply` to change anything) are tested against a fake account. Disposable workers are started by the coordinator when work waits and no worker serves the project; the launched host reads its token from the parameter store, never from user data. The Fly configuration passes `fly config validate` |
