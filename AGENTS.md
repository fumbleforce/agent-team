...
Retain atomic claims, single writer per worktree, one delivery per project, lease validation on every worker and tool call, fail-stop workers and scoped quarantine after uncertain execution. Never auto-retry or auto-reassign uncertain work. A turn cut off by a restart of its own worker is not uncertain: that worker ends the turn's process itself when it comes back and still holds the worktree, so the work continues there, and the same holds once that worker is seen asking for work again. A delivery cut off mid-merge is run again: the merge gate asks the host whether the approved change already merged, and merges only if it did not. A lost review is asked for again; it reads a throwaway checkout and changes nothing. Do not treat instructions or permission patterns as OS isolation. Publishing, merging and service activation require explicit authorization in the committed manifest.
