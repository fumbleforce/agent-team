...
- The coordinator's watchdog tells the owner about a quarantine itself, without a model: once per stall it comments on the project's `ownerInboxIssue` with the held job ids, each job's recorded outcome and summary, and the project's dashboard page, and once more when a job completes again. Repeated failures inside one stall add nothing, and the state survives a coordinator restart. It only notifies: requeue stays the operator's explicit act. `tracker.stallAlertAfter` sets how many held jobs it waits for (default 1, `0` turns it off); a project without `ownerInboxIssue`, or a coordinator without a tracker credential, is silently unaffected. The dashboard address comes from `dashboardUrl` in the coordinator configuration; without it the comment says so and reports everything else.
