...
test('team templates: save the team, export, import, create a team from one; hire from the library', async () => {
  ...
  try {
    ...
    assert.deepEqual((await call('/api/templates', { cookie })).json.items.map((item: any) => item.slug), ['imported', 'marketing-desk', 'research-desk', 'sales-desk', 'shop-team', 'the-originals'], 'the shipped teams sit beside what people saved');
