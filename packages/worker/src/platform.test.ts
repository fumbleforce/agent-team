...
test('a worker reports which engines it can start and which credential variables are set, by name only', () => {
  ...
  assert.deepEqual(readiness(engines, ['A_KEY', 'B_KEY'], { platform: 'linux', env: { PATH: `/nowhere:${dir}`, A_KEY: 'secret' } }), { engines: ['one'], variables: ['A_KEY'], models: {}, efforts: {} });
  assert.deepEqual(readiness(engines, ['A_KEY'], { platform: 'win32', env: { PATH: dir, PATHEXT: '.CMD', A_KEY: '' } }), { engines: ['two'], variables: [], models: {}, efforts: {} });
