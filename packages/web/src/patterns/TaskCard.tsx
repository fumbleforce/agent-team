...
export function TaskCard({ task, owner, roster = [], onAssign, href }: { task: TaskCardData; owner: Agent | undefined; roster?: Agent[]; onAssign?: ((taskId: string, agentId: string) => void) | undefined; href?: string | undefined }) {
  ...
  return (
    <Card as="article" tone="raised" pad="sm" className="flex flex-col gap-2">
      {href ? <Link href={href} className="rounded-chip hover:underline focus-visible:outline-2 focus-visible:outline-accent"><Text size="small" weight="medium">{task.title}</Text></Link> : <Text size="small" weight="medium">{task.title}</Text>}
      <div className="flex items-center gap-1.5">
        <Text size="caption" tone="muted" mono>{task.key}</Text>
        {task.tag && <Chip>{task.tag}</Chip>}
        {task.raised && <Chip tone="attention">{task.raised}</Chip>}
