...
export function parseEnqueueArgs(target, flags = []) {
  ...
  for (let i = 0; i < flags.length; i++) {
    ...
    if (flag === '--publish') body.publish = true;
      ...
      const key = { '--proposal-limit': 'proposalLimit', '--issue': 'issue', '--key': 'idempotencyKey', '--base': 'base', '--engine': 'engine', '--billing': 'billing', '--model': 'model', '--timeout-minutes': 'timeoutMinutes' }[flag];
...
export async function main(args = process.argv.slice(2)) {
  ...
  } else throw new Error('Usage: cli.mjs list | enqueue PROJECT [--issue ID [--approval-required]] [--publish [--auto-merge]] [--ideate --proposal-limit N] [--key KEY] [--base REF [--fetch]] [--engine NAME] [--billing MODE] [--model MODEL] [--timeout-minutes N] | cancel JOB | requeue JOB (only after human inspection)');
