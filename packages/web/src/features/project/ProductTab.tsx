...
interface Environment { id: string; name: string; branch: string | null; url: string }
...
export function ProductTab({ slug, navigate }: { slug: string; navigate(to: string): void }) {
  const view = useResource<{ environments: Environment[]; raised: Raised[] }>(`/api/projects/${slug}/product`);
  ...
  const paste = (event: ClipboardEvent) => { const file = [...event.clipboardData.files].find(item => item.type.startsWith('image/')); if (file) void upload(file).then(id => { setSnapshot(id); setMarkers([]); }, failure => setError((failure as Error).message)); };
  const send = async () => {
  ...
  return (
    <div className="flex min-h-0 grow" onPaste={paste}>
      <div className="flex min-w-0 grow flex-col gap-3 p-5">
        {environments.length > 0 && <div><Segmented value={environment?.id ?? ''} onChange={setChosen} options={environments.map(item => ({ value: item.id, label: item.branch ? `${item.name} · ${item.branch}` : item.name }))} /></div>}
      ...
      <SidePanel label="Snapshot and raised issues" side="right" wide>
        <div className="flex flex-col gap-2.5 px-1.5">
          ...
          {snapshot
            ...
            : <Text size="small" tone="muted">Take a screenshot of what you see and paste it here.</Text>}
          ...
          <div className="flex gap-1.5"><Button variant="primary" disabled={!snapshot || !text.trim()} onClick={() => { void send(); }}>Send to team</Button>{snapshot && <Button variant="ghost" onClick={() => { setSnapshot(null); setMarkers([]); }}>Discard</Button>}</div>
