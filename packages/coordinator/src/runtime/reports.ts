...
const NOTHING_SAID = new Set([
  ...
  'nothing to do', 'nothing to report', 'pass', 'passed', 'changes', 'fail', 'fails', 'failed', 'approved', 'approve', 'lgtm', 'ship it',
]);
export const saysNothing = (summary: string): boolean => NOTHING_SAID.has(summary.trim().toLowerCase().replace(/[.!?…]+$/, '').trim().replace(/\s+/g, ' '));
