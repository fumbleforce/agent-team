...
import { pollProject, runIntake } from './intake.mjs';

const manifest = { version: 1, name: 'Example', instructions: [], queueProjectId: 'example', workspaceId: 'workspace', workspaceUrl: 'https://tracker.example/w', projectId: 'project', projectUrl: 'https://tracker.example/w/project', teamId: 'team', readyLabel: 'agent:ready', delivery: { repository: 'owner/repo', autoMergeAuthorized: true }, ideation: { enabled: true, backlogCap: 10, batchSize: 3, minimumIntervalHours: 24, ideaLabel: 'Idea', proposedState: 'Backlog', approvedState: 'Todo', rejectedState: 'Canceled' } };
...
function harness({ ideas = [], jobs = [], remaining = 10, prepareError = false } = {}) {
  ...
  const tracker = { snapshot: async () => { reads++; return { ideas, remaining }; }, prepareApproved: async (_manifest, identifier) => { if (prepareError) throw new Error('Approval withdrawn'); prepared.push(identifier); } };
  ...
  return { poll: (overrides = {}) => pollProject({ manifest, tracker, queue, now: 100_000_000, ...overrides }), writes, prepared, jobs, reads: () => reads };
}
...
test('disabled ideation and API failures never reach queue', async () => {
  ...
  for (let i = 0; i < 2; i++) await assert.rejects(h.poll({ tracker: { snapshot: async () => { throw new Error('Authentication failed'); } }, queue: () => assert.fail('queue must not be called') }));
});
test('intake emits fixed bounded errors rather than underlying text', async () => {
  const errors = [];
  await assert.rejects(runIntake({ projects: { example: '/nonexistent/secret-test-key' } }, { once: true, tracker: {}, queue: () => assert.fail(), onError: error => errors.push(error.message) }), /Intake project poll failed/);
...
test('without local checkouts the intake polls manifests from the coordinator registry', async () => {
  ...
  const tracker = { snapshot: async () => ({ ideas: [], remaining: 0 }), prepareApproved: async () => {} };
  await runIntake({ coordinatorUrl: 'http://127.0.0.1:4310' }, { once: true, queue, tracker });
  assert.deepEqual(seen, []);
  await assert.rejects(runIntake({ projects: { example: 'relative' } }, { once: true, queue, tracker }), /absolute/);
});
