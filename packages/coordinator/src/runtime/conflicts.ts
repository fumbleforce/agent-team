...
const MERGE_SKILL = 'resolving-merge-conflicts';
...
export async function sendBackToMerge(tx: Tx, taskId: string, input: { now: number; approved: boolean; /* Only a task in one of these states is sent back. */ from?: readonly TaskState[]; actor: Pick<EventDraft, 'actorKind' | 'agentId' | 'turnId'>; drafts: EventDraft[] }): Promise<{ agentId: string; projectId: string; taskId: string } | null> {
  ...
  // The method for it, when the library has one.
  const method = await tx.selectFrom('versioned_docs').select('slug').where('kind', '=', 'skill').where('slug', '=', MERGE_SKILL).executeTakeFirst();
  const body = `${UPDATE_BRANCH(input.approved)}${method ? ` The ${MERGE_SKILL} skill has the method; read it with skill.read first.` : ''}`;
  await tx.insertInto('messages').values({ id: newId(input.now), thread_id: thread.id, author_kind: 'system', author_id: null, kind: 'system', body, payload: JSON.stringify({ unmergeableAt: task.head_sha }), created_at: input.now }).execute();
